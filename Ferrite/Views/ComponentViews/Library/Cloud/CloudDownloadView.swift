        ...
        DisclosureGroup("Downloads") {
            ForEach(filteredDownloads, id: \.self) { cloudDownload in
                CloudDownloadRow(
                    ...
                    onCopy: { cloudDownload in UIPasteboard.general.string = cloudDownload.link },
                    onOpen: { cloudDownload in if let url = URL(string: cloudDownload.link) { UIApplication.shared.open(url) } },
                    onDelete: { cloudDownload in pendingDeleteDownload = cloudDownload; showDeleteConfirm = true }
                )
